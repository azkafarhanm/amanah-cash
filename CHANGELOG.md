...
- Added Profile Photos Phase 2.2 Student Detail upload workflow for assigned Operators: mandatory keyboard-operable 1:1 crop and local unsaved preview, authoritative server validation and Sharp normalization to the four approved WebP renditions, private Vercel Blob storage through the provider-neutral `MediaStorage` boundary, and conflict-safe Student reference persistence. Failed writes preserve the prior reference and attempt immediate rollback of only the new objects; broad Student avatar rendering, deletion lifecycle, dashboards, Financial Assurance, and backup integration remain pending.
