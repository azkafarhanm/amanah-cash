...
export default async function OperatorsPage({ searchParams }: Props) {
  ...
  return <ContentWrapper>
    ...
    {result.items.length === 0 ? <EmptyState title="Belum ada Operator" description="Ubah filter atau tambahkan akun Operator pertama." /> : <>
      <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Nama</th><th>Email Google</th><th>Status</th><th>Dibuat</th><th>Login terakhir</th><th>Siswa</th></tr></thead><tbody>{result.items.map((operator) => <tr key={operator.id}><td><Link className={styles.link} href={`/admin/operators/${operator.id}`}>{operator.name}</Link></td><td>{operator.email}</td><td><StatusBadge tone={operator.isActive ? "success" : "neutral"}>{operator.isActive ? "Aktif" : "Tidak aktif"}</StatusBadge></td><td>{date(operator.createdAt)}</td><td>{date(operator.lastLoginAt)}</td><td>{operator.assignedStudentCount}</td></tr>)}</tbody></table></div>
