...
const FRAGMENT_SHADER = /* glsl */ `
  ...
  void main() {
    ...
    float mouseFalloff = exp(-mouseDist * 3.2); // Smooth organic exponential falloff
    float mouseRipple = sin(mouseDist * 9.5 - t * 2.0) * 0.5 + 0.5;
    vec2 mouseOffset = (uMouse - vec2(0.5)) * uMouseInfluence * uEnableMouse;

    // Dynamic UV warping with responsive gravitational lens
    vec2 warpedUv = uv + (mouseOffset * 0.55 + mouseDelta * mouseFalloff * uMouseInfluence * 0.85) * uEnableMouse;
    ...
    float bandCenter = uBandHeight + sin(warpedUv.x * 3.14159 * 0.75 + t * 0.22) * 0.12 
                       + n2 * 0.14 + (mouseOffset.y * 0.38 + mouseRipple * mouseFalloff * 0.08) * uEnableMouse;
    ...
    float cursorGlow = mouseFalloff * 0.22 * uEnableMouse;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const renderLoop = (now: number) => {
        ...
        // Smoothly damp mouse coordinates toward target
        if (!isMobile) {
          const mouseLerp = Math.min(delta * 5.5, 0.18);
