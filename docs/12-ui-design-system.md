...
|-----------------------|---------|
| `color.background.canvas`, `color.background.secondary`, `color.background.surface`, `color.background.subtle` | Page, secondary regions, and grouped content |
...
| `color.highlight.secondary`, `color.highlight.soft`, `color.highlight.background` | Sparse non-critical highlights |
...
| `color.info.foreground`, `color.info.background` | Informational content and callouts |
...
The approved palettes are defined only in `docs/18-design-tokens.md`. Each theme
maps the same semantic roles to its reviewed colors, while Deposit, Withdrawal,
and financial states retain explicit semantic families. Text and iconography
carry transaction meaning; color only reinforces it.
...
- Light, Dark, and System themes are approved for authenticated Settings. Dark
  uses the explicit semantic overrides in `docs/18-design-tokens.md`; it must
  never be generated by filter, inversion, opacity, or an ad hoc parallel
  palette.
- Light follows the approved Tropical Sunrise direction: a white-dominant
  canvas, quiet secondary areas, optional light-cyan surface separation, dark
  neutral text, teal interaction emphasis, and sparing orange or soft-yellow
  highlights. Financial status colors remain independent.
- Dark follows the approved Modern Tech & Finance direction: blue-slate
  surfaces, soft near-white text, restrained sky action emphasis, subtle
  elevation, and financial information stronger than decoration. Avoid pure
  black, pure white, neon, saturated decorative accents, heavy shadows, and
  excessive gradients.
- Light and Dark share typography, spacing, shape, component structure,
  iconography, interaction behavior, and semantic meaning. Only semantic color
  mappings change between themes.
...
| Settings | Sparse grouped preferences with immediate explanations and safe defaults; no settings for rules that must remain invariant. | Improves daily usability and recovery without weakening financial integrity. | Approved MVP; governed by `docs/21-mvp-settings-specification.md` |
...
- Any additional appearance preference beyond Light, Dark, and System.
- Any Dashboard, Reports, or Settings expansion beyond approved requirements.
