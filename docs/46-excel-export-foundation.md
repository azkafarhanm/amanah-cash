...
- places the document title, generated time, period, and existing summary items above the table in a consistent report hierarchy;
...
- applies a compact `Ringkasan` block, neutral bold table headers, frozen report/table context, transaction-table-only filters, wrapped cells, balanced semantic column widths, right alignment for display-ready currency/numeric cells, and print-ready A4 page settings; and
...
- report hierarchy, compact summary, frozen report/table context, transaction-only auto-filter, neutral table header, print settings, column-width bounds, and numeric alignment;
...
- Logos, branding content, covers, charts, graphs, signatures, formulas, multiple worksheets, advanced colors, conditional formatting, and premium layouts.
