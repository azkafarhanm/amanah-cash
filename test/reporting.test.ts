...
test("Reporting presentation is reusable, read-only, responsive, accessible, and export-neutral", () => {
  ...
  const sortLink = source("src/components/reports/report-sort-link.tsx");
  ...
  assert.match(components, /ariaLabel="Paginasi laporan"/);
  ...
  assert.match(components, /placeholder="Cari nama Siswa, catatan, alasan, atau operator"/);
  assert.match(components, /Urutkan hasil melalui judul kolom/);
  ...
  assert.match(filterForm, /router\.push\(href, \{ scroll: false \}\)/);
  assert.match(filterForm, /router\.replace\(hrefFromForm\(\), \{ scroll: false \}\)/);
  assert.doesNotMatch(filterForm, /<fieldset[^>]*disabled=\{pending\}/);
  assert.match(components, /<ReportSortLink/);
  assert.match(sortLink, /event\.currentTarget\.focus\(\{ preventScroll: true \}\)/);
  assert.match(sortLink, /event\.button !== 0/);
  assert.match(sortLink, /scroll=\{false\}/);
  assert.doesNotMatch(sortLink, /onClick|onKeyDown|tabIndex/);
  assert.match(filterForm, /}, 350\)/);
  ...
  assert.match(adminPage, /protectRoute\("admin"\)/);
