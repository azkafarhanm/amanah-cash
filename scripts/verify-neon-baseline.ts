...
async function main() {
  ...
  if (!directUrl || (!directUrl.startsWith("postgres://") && !directUrl.startsWith("postgresql://"))) {
    ...
    console.error("Example usage: DIRECT_URL=\"postgresql://...\" npx tsx scripts/verify-neon-baseline.ts");
    process.exit(1);
  }

  const isPostMigration = process.argv.includes("--post-migration");
  console.log(`=== PHASE 3: READ-ONLY NEON SCHEMA & DATA VERIFICATION (${isPostMigration ? "POST-MIGRATION" : "PRE-MIGRATION"}) ===`);
  console.log("Connecting to Neon database (read-only verification)...");
  ...
  try {
    ...
    console.log(`✔ PostgreSQL Version: ${serverInfo.rows[0].version.split(",")[0]}`);
    console.log(`✔ Current schema: ${serverInfo.rows[0].current_schema}`);

    // 1. Verify 9 Tables in 'public' schema
    console.log("\n1. Verifying 9 Application Tables...");
    ...
    // 2. Verify 6 Enums and their exact members
    console.log("\n2. Verifying 6 PostgreSQL Enums and values...");
    ...
    // 6. Verify Row Counts Across Application Tables
    const isPostMigration = process.argv.includes("--post-migration");
    console.log(`\n6. Verifying Row Counts across all 9 application tables (${isPostMigration ? "POST-MIGRATION MODE" : "PRE-MIGRATION MODE"})...`);

    const expectedPostMigrationCounts: Record<string, number> = {
      users: 4,
      settings_preferences: 3,
      accounts: 1,
      sessions: 0,
      operator_audit: 8,
      maintenance_audit_events: 1,
      students: 4,
      transactions: 42,
      financial_audit_events: 49
    };
    ...
    for (const table of expectedTables) {
      ...
      if (isPostMigration) {
        const expected = expectedPostMigrationCounts[table] ?? 0;
        assert.equal(count, expected, `Table '${table}' post-migration count mismatch: expected ${expected}, got ${count}`);
      } else {
    ...
    if (isPostMigration) {
      assert.equal(totalRows, 112, `Total post-migration rows mismatch: expected 112, got ${totalRows}`);
      const balanceRes = await client.query(`SELECT sum(balance)::bigint as total_balance FROM students;`);
      const totalBalance = BigInt(balanceRes.rows[0].total_balance);
      assert.equal(totalBalance, 519000n, `Total student balance mismatch: expected 519000, got ${totalBalance}`);
      console.log(`\n✔ Total rows across all tables: ${totalRows} (112 migrated rows confirmed)`);
      console.log(`✔ Total student balance in Neon: Rp ${totalBalance.toLocaleString("id-ID")} (Exact match Rp 519.000)`);
      console.log("\n=== ALL NEON POST-MIGRATION VERIFICATIONS PASSED SUCCESSFULLY ===");
    } else {
      assert.equal(totalRows, 0, `Total pre-migration rows mismatch: expected 0, got ${totalRows}`);
      console.log(`\n✔ Total rows across all tables: ${totalRows} (0 rows confirmed)`);
      console.log("\n=== ALL NEON PRE-MIGRATION VERIFICATIONS PASSED SUCCESSFULLY ===");
    }
